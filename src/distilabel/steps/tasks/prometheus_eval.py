...
class PrometheusEval(Task):
    ...
    outputs: StepColumns = ["feedback", "result", "model_name"]
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if self.mode == "absolute":
            if self.reference:
                self.inputs = ["instruction", "generation", "reference"]
            else:
                self.inputs = ["instruction", "generation"]
        else:
            if self.reference:
                self.inputs = ["instruction", "generations", "reference"]
            else:
                self.inputs = ["instruction", "generations"]
