...
class PrometheusEval(Task):
    ...
    inputs: List[str] = Field(
        default=["instruction", "generations", "reference"],
        frozen=True,
        description="The default inputs for the task are the 'instruction' and the 'generation(s)' if `reference=False`, otherwise, `reference` is required.",
    )
    outputs: List[str] = Field(
        default=["feedback", "result", "model_name"],
        frozen=True,
        description="The output for the task are the 'feedback' and the 'result' generated by Prometheus, as well as the 'model_name' which is automatically included based on the `LLM` used.",
    )
    ...
    def set_inputs(cls, values):
        ...
        if reference:
            inputs = ["instruction", "generations", "reference"]
        inputs = ["instruction", "generations"]
    ...
    def format_input(self, input: Dict[str, Any]) -> "ChatType":
        ...
        if self.mode == "absolute":
            if not isinstance(input["generations"], str):
                raise ValueError(
                    f"Provided `generations` is of type {type(input['generations'])} but a string"
                    " should be provided instead.",
                )

            template_kwargs["generation"] = input["generations"]
            system_message = (
