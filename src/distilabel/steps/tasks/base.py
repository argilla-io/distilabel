...
def iterate_generations_with_stats(
    ...
    """
    extra_keys = [
        key for key in output.keys() if key not in ("generations", "statistics")
    ]
    for i, generation in enumerate(output["generations"]):
        # Create a new dictionary with the statistics for this index
        stats = {key: values[i] for key, values in output["statistics"].items()}  # type: ignore
        # Extra keys returned by the `LLM`
        extra = {key: values[i] for key in extra_keys for values in output[key]}
