...
def execute_from_response(
    ...
    It will run 2 attempts to cast the arguments to the correct types, one assuming
    the parameters are named as expected, and in case of error another one just assuming
    the positional arguments can work.
    ...
    try:
        call_answer = {
            name: param_types[name](value) for name, value in call_answer.items()
        }
    except (KeyError, ValueError):
        try:
            call_answer = {
                name: param_type(value)
                for (param_name, param_type), (name, value) in zip(
                    param_types.items(), call_answer.items()
                )
            }
        except ValueError:
