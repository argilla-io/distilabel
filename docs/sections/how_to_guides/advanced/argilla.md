...
!!! NOTE
    Note that a [`Pipeline`][distilabel.pipeline.Pipeline] outputs a [`Distiset`][distilabel.distiset.Distiset], which is a `datasets.DatasetDict` with multiple configurations depending on the leaf nodes of the [`Pipeline`][distilabel.pipeline.Pipeline].
